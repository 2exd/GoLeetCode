...
func target(id string, job func()) (count int) {
	...
	if !exists {
		...
		job() // Perform the operation

		close(op.doneChan)

		lock.Lock()
		finalCount := int(atomic.LoadInt32(&op.count)) // Safely load the count
		delete(operations, id)
		lock.Unlock()

		return finalCount
	} else {
		atomic.AddInt32(&op.count, 1) // Safely increment the count
		lock.Unlock()

		<-op.doneChan // Wait for the operation to complete

		return int(atomic.LoadInt32(&op.count)) // Return the final count safely
	}
}
